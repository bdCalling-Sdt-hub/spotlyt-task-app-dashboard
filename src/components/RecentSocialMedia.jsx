...
const RecentSocialMedia = () => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [client, setClient] = useState();
  const {data:social,isLoading,isSuccess} = useGetSocialQuery();
  console.log(social);
  ...
  return (
    <div className="shadow-2xl">
      <div className="flex justify-between items-center text-white bg-[#318130] rounded-t-2xl px-[15px] py-[20px]">
        <p>Social Media</p>
        <Link to="/tasks/social-media">See All</Link>
      </div>
...
>
<Table  pagination={false} columns={columns} dataSource={social?.data?.attributes?.tasks.slice(0,4)} />
</ConfigProvider>
...
export default RecentSocialMedia;
